...
class AddNewQuestEntity (
    private val repo: LocalQuestsRepository
        ...
        operator fun invoke(title: String,description:String, author: String) = repo.insertQuestEntity(QuestEntity(title =title,description=description, author = author).also{ Log.d("USE CASE",
            it.toString()
        )})
